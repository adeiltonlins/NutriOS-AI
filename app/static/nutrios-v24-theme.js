(function(){
  ...
  window.NutriOSTheme={apply,get:()=> 'light',toggle:apply};
  apply();
  ...
  function setupProfessionalShell(){
    ...
    addStylesheet('/static/nutrios-universal-light.css?v=1','universal-light');
    if(p==='/app'){
    ...
    if(!window.NutriOSUI&&!document.querySelector('script[data-nutrios-app-shell]')){
      const js=document.createElement('script');js.src='/static/nutrios-app-shell.js?v=4';js.defer=true;js.dataset.nutriosAppShell='1';document.head.appendChild(js);
    }
  }
  setupProfessionalShell();

  async function setupTenantPWA(){
    const match=location.pathname.match(/^\/n\/([^/]+)$/);if(!match)return;
    const slug=decodeURIComponent(match[1]),fallback='/static/icons/icon-512.svg';
    try{
      const r=await fetch('/public/clientes/'+encodeURIComponent(slug),{credentials:'same-origin'});if(!r.ok)throw new Error('identity');
      const cfg=await r.json(),name=String(cfg.nome||'NutriOS').slice(0,80),color=/^#[0-9a-fA-F]{6}$/.test(String(cfg.cor_principal||''))?cfg.cor_principal:'#163832',icon=String(cfg.logo_url||fallback);
      const manifest={name:name+' — NutriOS',short_name:name.slice(0,24),description:'Atendimento nutricional digital',start_url:location.pathname,scope:location.pathname.replace(/\/$/,'')+'/',display:'standalone',background_color:'#f7f6f2',theme_color:color,orientation:'portrait-primary',lang:'pt-BR',icons:[{src:icon,sizes:'192x192',type:'image/png',purpose:'any'},{src:icon,sizes:'512x512',type:'image/png',purpose:'any'},{src:icon,sizes:'512x512',type:'image/png',purpose:'maskable'}]};
      const blob=new Blob([JSON.stringify(manifest)],{type:'application/manifest+json'}),url=URL.createObjectURL(blob);let link=document.querySelector('link[rel="manifest"]');if(!link){link=document.createElement('link');link.rel='manifest';document.head.appendChild(link)}link.href=url;document.documentElement.style.setProperty('--tenant-color',color);
  ...
  function mount(){
    document.querySelectorAll('#nutrios-theme-toggle,#themeToggle,[data-theme-toggle]').forEach(el=>el.remove());
    apply();setupTenantPWA();
  }
