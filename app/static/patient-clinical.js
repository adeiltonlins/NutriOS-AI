(() => {
  ...
  async function loadClinical() {
    ...
    mealPlan.innerHTML = plan ? `<h3>${esc2(plan.title)}</h3><p>${esc2(plan.objective || "")}</p>${(plan.content || []).map((meal) => `<article class="document"><b>${esc2(meal.name)}</b><ul>${meal.items.map((item) => `<li>${esc2(item.name)} — ${item.grams} g</li>`).join("")}</ul></article>`).join("")}<p><b>Total aproximado: ${plan.totals?.kcal || 0} kcal</b></p><p>${esc2(plan.patient_notes || "")}</p>` : '<p class="muted">Seu nutricionista ainda não publicou um plano.</p>';
